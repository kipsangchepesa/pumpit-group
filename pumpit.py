...
st.title('Pumpit Up Group MLApp')
...
class Predictor:
    ...
    def set_classifier_properties(self):
        ...
        if self.type == "Regression":
            ...
            self.chosen_classifier = st.sidebar.selectbox("Please choose a classifier", ('Logistic Regression', 'Random Forest','Naive Bayes', 'Neural Network')) 
            if self.chosen_classifier == 'Logistic Regression': 
                ...
                self.max_depth = st.sidebar.slider('max_depth', 1, 6, 2)
                self.random_state =st.sidebar.slider('random_state',1,42,7)
...
if __name__ == '__main__':
    ...
    try:
        ...
        st.markdown('<span style="color:blue">Please choose the right csv file with target</span>', unsafe_allow_html=True)  
